...
function Controller() {
    ...
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.apiPrefix = (this.environment === "development") ? "//10.0.18.79:8134/actOrder/" : "/"; //api接口地址前缀    
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    系统各个模块API地址
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.apiUrl = {
        "wechat": {
            "getPhoneVertifyCode": this.apiPrefix + "getPhoneVertifyCode.rest", //获取验证码
            "saveData": this.apiPrefix + "saveData.rest" //保存提交的数据
        }
    };
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    出错时标记
    ...
    this.showLog = function(msg, callback) {
        var classSelf = this;
        if (classSelf.toastStatus) {
            classSelf.toastStatus = false;
            var errElm = $('.wk-toast');
            if (!errElm[0]) {
                errElm = $('<div class="wk-toast"></div>')
                    .appendTo('body');
            }
    ...
    this.checkMobile = function(mobile) {
        var result = true;
        if (!(/^(13[0-9]|14[6|7]|15[0|1|2|3|5|6|7|8|9]|18[0-9]|17[0-9])\d{8}$/.test(mobile))) {
            result = false;
        }
        return result;
    };
