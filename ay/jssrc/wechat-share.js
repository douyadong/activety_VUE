/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 4. 备注：对api的依赖：jQuery
 -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
function WechatShareController(opts) {
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    相关配置参数
    opts.title :分享的标题
    opts.content : 分享的内容
    opts.linkUrl : 分享的链接
    opts.imgUrl: 分享的图片地址
    opts.canShareTimeline 可以分享到朋友圈
    opts.canShareAppMessage 可以发送给微信好友
    opts.wxConfigCallback 微信config 函数执行完成后的回调
    opts.wxReadyCallback 微信ready函数的执行的回调
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.opts = opts || {};
    ...
    分享的标题
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.opts.title = opts.title || $("#wechatTitle").val();
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    分享的内容
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.opts.content = opts.content || $("#wechatContent").val();
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    分享的链接
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.opts.linkUrl = opts.linkUrl || window.location.href;
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    分享的图片
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    this.opts.imgUrl = opts.imgUrl || 'http:' + $("#wechatImgUrl").val();
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    全局的环境地址
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    this.onload = function () {
        ...
        wx.ready(function () {
            ...
            if (classSelf.opts.canShareTimeline) {
                classSelf.wx_onMenuShareTimeline(classSelf.opts.title, classSelf.opts.linkUrl, classSelf.opts.imgUrl); //  监听“分享到朋友圈
            }
            else {
                wx.hideMenuItems({
            ...
            if (classSelf.opts.canShareAppMessage) {
                classSelf.wx_onMenuShareAppMessage(classSelf.opts.title, classSelf.opts.linkUrl, classSelf.opts.content, classSelf.opts.imgUrl); //  监听“发送给朋友”按钮点击
            }
            else {
                wx.hideMenuItems({
